...
template <typename scalar_t, typename accscalar_t, bool increasing_stride, int log_max_stride,
          typename Function0, typename Function1, typename Function2>
__global__ void butterfly_ortho_multiply_untied_forward_backward_cuda_kernel(const CudaAcsr<scalar_t, 3> twiddle_cos_a,
                                                                             const CudaAcsr<scalar_t, 3> twiddle_sin_a,
                                                                             Function0 load_input,
                                                                             Function1 load_grad,
...
void butterfly_ortho_multiply_untied_forward_backward_cuda(const at::Tensor& twiddle_cos, const at::Tensor& twiddle_sin, const at::Tensor& input,
  ...
  const int n = input.size(2);
  const int log_n = int(log2((double) n));
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "butterfly_ortho_multiply_untied_forward_backward_cuda", [&] {
    using accscalar_t = at::acc_type<scalar_t, true>;
    const auto twiddle_cos_a = twiddle_cos.packed_accessor<scalar_t, 3, at::RestrictPtrTraits, int32_t>();
    const auto twiddle_sin_a = twiddle_sin.packed_accessor<scalar_t, 3, at::RestrictPtrTraits, int32_t>();
    const auto input_a = input.packed_accessor<scalar_t, 3, at::RestrictPtrTraits, int32_t>();
    ...
    switch (log_stride)
      {
      case 0:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 0>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 0>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 1:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 1>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 1>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 2:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 2>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 2>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 3:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 3>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 3>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 4:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 4>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 4>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 5:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 5>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 5>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 6:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 6>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 6>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 7:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 7>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 7>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 8:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 8>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 8>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      case 9:
        increasing_stride ? butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, true, 9>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size)
                          : butterfly_ortho_multiply_untied_forward_backward_cuda_kernel<scalar_t, accscalar_t, false, 9>
          <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(twiddle_cos_a, twiddle_sin_a, load_input, load_grad, d_twiddle_a, save_d_input, batch_size); break;
      }
  });
  AT_CHECK(cudaGetLastError() == cudaSuccess,
     "butterfly_ortho_multiply_untied_forward_backward_cuda failed with error code ",
     cudaGetLastError());
}
...
void bbt_conv2d_cuda(const at::Tensor& twiddle, const at::Tensor& input,
    at::Tensor& output, const int kernel_size, const int padding,
    const int h_out, const int w_out) {
  int batch_size = output.size(0); // b_out = b_in * h_out * w_out
  const int stack = kernel_size * kernel_size;
  const int nstack = twiddle.size(0);
  const int n = input.size(1); // c_in
  const int log_n = int(log2((double) n));
  const int h_in = input.size(2);
  const int w_in = input.size(3);
  const int c_out_ratio = nstack / stack;
  const int nblocks = twiddle.size(1) / (2 * log_n);
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "bbt_conv2d_cuda", [&] {
    using accscalar_t = at::acc_type<scalar_t, true>;
    const auto twiddle_a = twiddle.packed_accessor<scalar_t, 5, at::RestrictPtrTraits, int32_t>();
    ...
    auto load_input = [batch_size, stride, input_a, kernel_size,
      padding, h_out, w_out, h_in, w_in] __device__ (scalar_t* s_input) {
      const int b = blockIdx.x * blockDim.y + threadIdx.y;
    ...
    auto save_output = [batch_size, stride, output_a] __device__ (scalar_t* s_input) mutable {
      ...
      // stack (s) here is kernel_size*kernel_size*c_out_ratio
      if (b < batch_size) {
        for (int i = threadIdx.x; i < stride * 2; i += blockDim.x) {
          output_a[b][s][i] = s_input[i + threadIdx.y * stride * 2];
        }
...
void bbt_conv2d_forward_backward_cuda(const at::Tensor& twiddle,
  ...
  const int w_in = input.size(3);
  int nblocks = twiddle.size(1) / (2 * log_n);
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "bbt_conv2d_forward_backward_cuda", [&] {
    using accscalar_t = at::acc_type<scalar_t, true>;
    const auto twiddle_a = twiddle.packed_accessor<scalar_t, 5, at::RestrictPtrTraits, int32_t>();
    const auto input_a = input.packed_accessor<scalar_t, 4, at::RestrictPtrTraits, int32_t>();
    const auto grad_a = grad.packed_accessor<scalar_t, 3, at::RestrictPtrTraits, int32_t>();
