...
def get_parser():
    ...
    parser.add_argument('--mapping', type=str, help='Text file with names of layers to file paths')
    parser.add_argument('--out', default='.', help='Output directory for initialized models with butterflies.')
...
student_model = models.__dict__[args.arch](num_structured_layers=args.num_structured_layers,
    structure_type=args.structure_type, nblocks=args.nblocks, param=args.param)
print(student_model.state_dict().keys())
checkpoint = torch.load(args.pretrained, map_location = lambda storage, loc: storage.cuda())
loaded_state_dict = checkpoint['model']

with open(args.mapping, 'r') as f:
    layers_to_replace = f.readlines()
    for layer_to_replace in layers_to_replace:
        name, file, key = layer_to_replace.strip().split()
        matrix = torch.load(file)['model']
        print(matrix.keys())
        if args.structure_type == 'LR':
            loaded_state_dict[f"{name}.{key}"] = matrix[key]
        elif args.structure_type == 'BBT':
            for i in range(args.nblocks*2):
                loaded_state_dict[f'{name}.layers.{i}.twiddle'] =  matrix[f'layers.{i}.twiddle']
        else:
             loaded_state_dict[f'{name}.twiddle'] =  matrix['twiddle']
        try:
            del loaded_state_dict[f"{name}.weight"]
        except:
            print("Already deleted key.")

student_model.load_state_dict(loaded_state_dict, strict=True)
torch.save(student_model.state_dict(), args.out)
