...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = x.view(-1, 3, 1024)
        y0 = self.fc100(x[:,0,:]) + self.fc110(x[:,1,:]) + self.fc120(x[:,2,:])
        y1 = self.fc101(x[:,0,:]) + self.fc111(x[:,1,:]) + self.fc121(x[:,2,:])
        y2 = self.fc102(x[:,0,:]) + self.fc112(x[:,1,:]) + self.fc122(x[:,2,:])
        x = self.fc20(y0) + self.fc21(y1) + self.fc22(y2)
        x = F.relu(x)
        x = F.relu(self.fc3(x))
        x = F.relu(self.fc4(x))
        x = self.logits(x)
